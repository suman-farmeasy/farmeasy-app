...
class ListLandOwnerViewModel{
  ...
  Future<ListLandOwnerResponseModel>listLandOwner(int currentPage,String searchWord,var headerMap,)async{
    dynamic response = await _api.getApi(ApiUrls.LIST_LAND_OWNER+'page=$currentPage&search=$searchWord', true, headerMap);
    return ListLandOwnerResponseModel.fromJson(response);
  }
}
class ListAllLandViewModel{
  final _api = NetworkApiServices();
  Future<AllLandResponseModel>listlands(int currentPage, String searchData, var headerMap,)async{
    dynamic response = await _api.getApi(ApiUrls.LIST_ALL_LAND+'$currentPage&search=$searchData', true, headerMap);
