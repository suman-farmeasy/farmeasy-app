...
class GetProfileDetailsViewModel{
  ...
  Future<GetProfileResponseModel>getProfile(headerMap) async{
    dynamic response = await _api.getApi(ApiUrls.GET_PROFILE_DETAILS, true , headerMap);
...
class UpdateProfileViewModel{
  final _api = NetworkApiServices();
  Future<UpdateProfileResponseModel>updateProfile(var headerMap,var data)async{
    dynamic response = await _api.patchApi(ApiUrls.UPDATE_PROFILE, data, true,headerMap);
...
class EducationListViewModel{
  ...
  Future<EducationListResponseModel>getEducationData(String search)async{
    dynamic response = await _api.getApi(ApiUrls.EDUCATION_LIST+'$search', false,{});
