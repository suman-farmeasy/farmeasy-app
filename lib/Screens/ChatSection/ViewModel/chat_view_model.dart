...
class ChatViewModel {
  ...
  Future<ChatsResponseModel> chatData(
      var headermap, int id, int currentPage) async {
    dynamic response = await _api.getApi(
        ApiUrls.CHAT_DATA + '$id&page=$currentPage', true, headermap);
...
class SendMessageViewModel {
  ...
  Future<SendMessageResponseModel> sendMessage(var headerMap, var data) async {
    dynamic response =
        await _api.postApi(ApiUrls.SEND_MESSGAE, data, true, headerMap);
...
class IsMessageSeenViewModel {
  ...
  Future<MessageSeenResponseModel> isMessageSeen(var headerMap, int id) async {
    dynamic response =
        await _api.putApi(ApiUrls.IS_MESSAGE_SEEN + '$id', true, headerMap);
