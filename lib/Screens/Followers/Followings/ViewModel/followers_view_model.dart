...
class FollowingListViewModel{
  ...
  Future<FollowingListResponseModel>followersList(var headerMap, int userId, int currentPage)async{
    dynamic response = await _api.getApi(ApiUrls.FOLLOWING_LIST+'$userId&page=$currentPage', true, headerMap);
    return FollowingListResponseModel.fromJson(response);
  }
}
class FollowUnfollowViewModel{
  final _api = NetworkApiServices();
  Future<FollowUnfollowResponseModel>followersList(var headerMap,var data)async{
    dynamic response = await _api.postApi(ApiUrls.FOLLWO_UNFOLLOW,data, true, headerMap);
