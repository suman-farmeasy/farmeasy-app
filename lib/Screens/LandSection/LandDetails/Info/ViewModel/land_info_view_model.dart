...
class LandDetailViewModel {
  ...
  Future<LandDetailsResponseModel> landInfo(var headerMap, int id) async {
    dynamic response = await _infoService.getApi(
      ApiUrls.GET_LAND_DETAILS + "${id}",
      true,
      headerMap,
    );
...
class CheckLandDetailsViewMOdel {
  ...
  Future<CheckLandDetailsResponseModel> landDetails(
      var headerMap, int id) async {
    dynamic response =
        await _api.getApi(ApiUrls.CHECK_LAND_DETAILS + '$id', true, headerMap);
...
class UpdateLandDetailsViewModel {
  final _api = NetworkApiServices();
  Future<LandUpdateResponseModel> landUpdate(
      var headerMap, int id, var data) async {
    dynamic response = await _api.patchApi(
        ApiUrls.LAND_DETAILS_UPDATE + '$id', data, true, headerMap);
...
class UploadLandImagesViewModel {
  ...
  Future<LandImageResponseModel> imgUpdate(var headerMap, var data) async {
    dynamic response =
        await _api.patchApi(ApiUrls.UPDATE_LAND_IMAGES, data, true, headerMap);
...
class CropSuggestionViewModel {
  final _api = NetworkApiServices();
  Future<CropSuggestionResponseModel> cropSuggestionList(
      var headerMap, int landId) async {
    dynamic response =
        await _api.getApi(ApiUrls.CROP_SUGGESTION + '$landId', true, headerMap);
...
class ChatGPTCropSuggestionViewModel {
  final _api = NetworkApiServices();
  Future<ChatGptCropSuggestionResponseModel> cropSuggestionList(
      var headerMap, String city, String state, String country) async {
    dynamic response = await _api.getApi(
        ApiUrls.SUGGEST_CROP + 'city=$city&state=$state&country=$country',
        true,
        headerMap);
