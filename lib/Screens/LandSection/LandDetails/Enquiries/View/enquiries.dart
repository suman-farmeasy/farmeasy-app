        ...
        child: Padding(
          ...
          child: Obx(() {
            return controller.loading.value
              ...
              child: controller.enquiriesData.value.result?.data?.length !=0?Container(
                height: AppDimension.h * 0.77,
                child: ListView.builder(
                    controller: controller.scrollController,
                    itemCount: controller.enquiriesData.value.result?.data?.length ?? 0,
                    itemBuilder: (context, index) {
                      return InkWell(
                        onTap: (){
                          Get.to(()=>ChatScreen(
                            landId: controller.landId.value,
                            enquiryId: controller.enquiriesData.value.result!.data![index].id!.toInt(),
                            userId: controller.enquiriesData.value.result!.data![index].connectedPersonUserId!.toInt(),
                            userName: controller.enquiriesData.value.result!.data![index].connectedPersonName??"",
                            userFrom: controller.enquiriesData.value.result!.data![index].connectedPersonAddress??"",
                            userType: controller.enquiriesData.value.result!.data![index].connectedPersonUserType??"",
                            image: controller.enquiriesData.value.result!.data![index].connectedPersonImage??"",
                            enquiryData: controller.enquiriesData.value.result!.data![index].created??"",
                            isEnquiryCreatedByMe: controller.enquiriesData.value.result!.data![index].isCreatedByMe??false,
