...
class CurrentWeatherViewModel {
  ...
  Future<CurrentWeatherResponseModel> currentWeather(
      double latitude, double longitude, String key) async {
    dynamic response = await _api.getApi(
        ApiUrls.CURRENT_WEATHER_API +
...
class WeatherForecastViewModel {
  final _api = NetworkApiServices();
  Future<WeatherForecastResponseModel> weatherForecast(
      double latitude, double longitude, String key) async {
    dynamic response = await _api.getApi(
        ApiUrls.WEATHER_FORECAST +
            'lat=$latitude&lon=$longitude&appid=$key&cnt=8&units=metric',
        false,
        {});
